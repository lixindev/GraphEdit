...
export default {
    ...
    mouseDownEventHandler: function(event){
        ...
        if(this.selectedElement){
            if(this.selectedElement instanceof Relation){
                ...
                if(isHitLineController){
                    ...
                    if (isHitLineController.operation === Settings.lineOperations.startPointChange || isHitLineController.operation === Settings.lineOperations.endPointChange){
        ...
        if(isHitConnector){
            ...
            if (hitItem.length === 0){
                ...
                if(!this.selectedElement || !(this.selectedElement instanceof Group)
                    || (this.selectedElement.nodes.indexOf(topItem) === -1 && this.selectedElement.relations.indexOf(topItem) === -1)){
    ...
    changeStartOrEndOfLine: function(event){
        ...
        if(this.lineCache.operation === Settings.lineOperations.startPointChange){
        ...
        } else if (this.lineCache.operation === Settings.lineOperations.endPointChange){
            if(collection.length > 0){
                collection.sort(sortCon);
                const conNode = collection.pop();
    ...
    drawNewDashLine: function(event){
        ...
        if(!this.currentLine){
            ...
            this.currentLine = EditorFunction.createRelation.call(this, params);
        } else {
        ...
        EditorFunction.rollbackForMoving.call(this);
        EditorFunction.saveAll.call(this);
        this.currentLine.drawDashLine(Settings.lineOperations.endPointChange);
    },
    ...
    drawLine: function(event){
        ...
        //变更线条路径
            ...
            this.indexMark ++;
            if (this.lineCache.operation !== Settings.lineOperations.middlePathChange){
                //没变化
                ...
                if (this.lineCache.from){
                    this.lineCache.from.relations.splice(this.lineCache.from.relations.findIndex(relation => relation === this.currentLine), 1);
                };
                if (this.lineCache.to){
                    this.lineCache.to.relations.splice(this.lineCache.to.relations.findIndex(relation => relation === this.currentLine), 1);
                };
                if (this.currentLine.from){
                    this.currentLine.startPoint = null;
                    pushHashSet(this.currentLine.from.relations, this.currentLine);
                } else if (this.currentLine.operation === Settings.lineOperations.startPointChange) {
                    this.currentLine.startPoint = point;
                };
                if (this.currentLine.to){
                    this.currentLine.endPoint = null;
                    pushHashSet(this.currentLine.to.relations, this.currentLine);
                } else if (this.currentLine.operation === Settings.lineOperations.endPointChange) {
                    this.currentLine.endPoint = point;
                }; 
                this.currentLine.createPath();
            } else {
    ...
    mouseOverSelect: function(event){
        if(this.selectedElement){
            ...
            if(this.selectedElement instanceof Relation){
                let isHitLineController = false;
                isHitLineController = this.selectedElement.line.controller.hitCheck(event.offsetX * Settings.ratio, event.offsetY * Settings.ratio);
                if(isHitLineController){
                    if (isHitLineController.operation === Settings.lineOperations.startPointChange || isHitLineController.operation === Settings.lineOperations.endPointChange){
                        this.canvas.style.cursor = 'crosshair';
        ...
        if (!isHitConnector){
            ...
            if (hitItem.length === 0){
                ...
                } else if (top instanceof Relation){
                    this.canvas.style.cursor = 'default';
                }
    ...
    nodeResizing: function(event){
        ...
        this.selectedElement.relations.map(relation => {
            relation.createPath();
    ...
    keyDown: function(event){
        ...
        //移动节点或组
            ...
            event.preventDefault();
            EditorFunction.moveShapeOrGroup.call(this, left);
        } else if (event.keyCode === 38) {
            event.preventDefault();
            EditorFunction.moveShapeOrGroup.call(this, up);
        } else if (event.keyCode === 39) {
            event.preventDefault();
            EditorFunction.moveShapeOrGroup.call(this, right);
        } else if (event.keyCode === 40) {
            event.preventDefault();
            EditorFunction.moveShapeOrGroup.call(this, down);
        }
    },
    //画布大小调节
    ...
    moveCanvasOrCompose: function(event){
        ...
        if(this.composeModel && event.ctrlKey){
                ...
                for (const element of this.elements) {
                    ...
                    if(element instanceof Node){
                        ...
                        if (element.line.inflexionPoint){
                            ele.inflexionPoint = CommonFunctions.deepClone(element.line.inflexionPoint);
                        }
                        this.canvasMoveCache.push(ele);
                    }
                }
            ...
            for (const ele of this.canvasMoveCache) {
                let element;
                if (Object.hasOwnProperty.call(this.elementDict, ele.id)){
                    element = this.elementDict[ele.id];
                    if (element instanceof Node) {
                        element.shape.x = ele.x + x;
                        element.shape.y = ele.y + y;
                    } else if (element instanceof Relation) {
                        if (!element.from) {
                            element.startPoint.x =  ele.startPoint.x + x;
                            element.startPoint.y =  ele.startPoint.y + y;
                        }
                        if (!element.to) {
                            element.endPoint.x =  ele.endPoint.x + x;
                            element.endPoint.y =  ele.endPoint.y + y;
                        }
                        if (element.line.inflexionPoint) {
                            element.line.inflexionPoint = [];
                            for (const point of ele.inflexionPoint) {
                                const newpoint = {};
                                newpoint.x = point.x + x;
                                newpoint.y = point.y + y;
                                element.line.inflexionPoint.push(newpoint);
                            }
                        }
                    }
                }
    ...
    moveCanvasComplete: function(event){
        ...
        if(this.composeModel){
            ...
            const composeNodes = this.nodes.filter(node => top <= node.shape.top && left <= node.shape.left && right >= node.shape.right 
            ...
            if(composeNodes.length === 1){
                ...
                this.selectedElement.relations = this.relations.filter(relation => composeNodes.indexOf(relation.from) !== -1 
                && composeNodes.indexOf(relation.to) !== -1);
                EditorFunction.reload.call(this);
            }
            //触发选中元素事件
            let selectedElementEvent = Event.selectedElement;
    ...
    //拖动新节点进入画布
    mouseEnterEventHandler: function(event) {
        this.removeEventListener('mouseenter', this.mouseEnterEventHandler);
        this.removeEventListener('mousemove', this.mouseOverSelect);
        this.clickPoint.x = event.offsetX * Settings.ratio;
        this.clickPoint.y = event.offsetY * Settings.ratio;
        this.dragedNode.x = event.offsetX * Settings.ratio;
        this.dragedNode.y = event.offsetY * Settings.ratio;
        this.selectedElement = EditorFunction.createNode.call(this, this.dragedNode, true);
