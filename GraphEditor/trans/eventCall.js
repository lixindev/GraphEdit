...
exports.default = {
    ...
    mouseDownEventHandler: function mouseDownEventHandler(event) {
        ...
        if (this.selectedElement) {
            if (this.selectedElement instanceof _line.Line) {
        ...
        if (isHitConnector) {
            ...
            if (hitItem.length === 0) {
                ...
                //     };    
                // }
                if (!this.selectedElement || !(this.selectedElement instanceof _group.Group) || this.selectedElement.nodes.indexOf(topItem) == -1 && this.selectedElement.lines.indexOf(topItem) == -1) {
    ...
    drawNewDashLine: function drawNewDashLine(event) {
        ...
        if (!this.currentLine) {
            ...
            this.currentLine = _editorFunction2.default.createLine.call(this, params);
        } else {
    ...
    drawDashShape: function drawDashShape(event) {
        ...
        //步骤一：还原区域图形
        _editorFunction2.default.rollbackForMoving.call(this);
        if ((0 <= x && x < minMovement || minMovement * -1 < x && x < 0) && (0 <= y && y < minMovement || minMovement * -1 < y && y < 0)) return;
            ...
            if (isHitTop) {
                if (isHitBottom) {
                    if (top.shape.center.x === bottom.shape.center.x) {
                        ...
                        //上下图形中心点都进入了偏移范围
                        var s_top = offset_top < 0 ? offset_top * -1 : offset_top;
                        var s_bottom = offset_bottom < 0 ? offset_bottom * -1 : offset_bottom;
                        //上边优先，谁近向谁靠拢
                        if (s_bottom < s_top) {
            ...
            if (isHitRight) {
                if (isHitLeft) {
                    if (right.shape.center.y === left.shape.center.y) {
                        ...
                        //左右图形中心点都进入了偏移范围
                        var s_right = offset_right < 0 ? offset_right * -1 : offset_right;
                        var s_left = offset_left < 0 ? offset_left * -1 : offset_left;
                        //左边优先，谁近向谁靠拢
                        if (s_right < s_left) {
                            this.offset_Y = offset_right;
        ...
        if (this.currentLine && !this.lineCache) {
            ...
            if (this.lineCache.operation !== 'middlePathChange') {
                ...
                if (this.lineCache.from) {
                    this.lineCache.from.lines.splice(this.lineCache.from.lines.findIndex(function (line) {
                        return line === _this.currentLine;
                    }), 1);
                };
                if (this.lineCache.to) {
                    this.lineCache.to.lines.splice(this.lineCache.to.lines.findIndex(function (line) {
                        return line === _this.currentLine;
                    }), 1);
                };
    ...
    mouseOverSelect: function mouseOverSelect(event) {
        if (this.selectedElement) {
            ...
            if (this.selectedElement instanceof _line.Line) {
                var isHitLineController = false;
                isHitLineController = this.selectedElement.controller.hitCheck(event.offsetX * _settings2.default.ratio, event.offsetY * _settings2.default.ratio);
                if (isHitLineController) {
                    if (isHitLineController.operation === 'startPointChange' || isHitLineController.operation === 'endPointChange') {
                        this.canvas.style.cursor = 'crosshair';
        ...
        if (!isHitConnector) {
            ...
            if (hitItem.length === 0) {
                ...
                } else if (top instanceof _line.Line) {
                    this.canvas.style.cursor = 'default';
                }
            };
            hitItem = [];
    ...
    nodeResizing: function nodeResizing(event) {
        ...
        var shapeType = this.selectedElement.shape.type;
        ...
        this.selectedElement.lines.map(function (line) {
            line.createPath();
            line.index = _this2.indexMark;
            _this2.indexMark++;
        });
        _editorFunction2.default.reload.call(this);
    },
    nodeResize: function nodeResize(event) {
        if (this.stepMark < this.stepCache.length - 1) {
            this.stepCache.splice(this.stepMark + 1, this.stepCache.length - this.stepMark - 1);
        }
        this.stepCache.push(_editorFunction2.default.outPut(this.elements));
    ...
    resize: function resize(event) {
        var width = document.documentElement.clientWidth * 7 / 12 - 25;
        ...
        if (this.composeModel && event.ctrlKey) {
            ...
            if (!this.canvasMoveCache) {
                ...
                try {
                    for (var _iterator9 = this.elements[Symbol.iterator](), _step9; !(_iteratorNormalCompletion9 = (_step9 = _iterator9.next()).done); _iteratorNormalCompletion9 = true) {
                        ...
                        } else if (element instanceof _line.Line) {
                            ele.id = element.id;
            ...
            try {
                var _loop = function _loop() {
                    ...
                    var x_original = void 0,
                        y_original = void 0;
                    if (element instanceof _node3.Node) {
                        _this3.canvasMoveCache.map(function (ele) {
                        ...
                        if (element.inflexionPoint) {
                            ...
                            _this3.canvasMoveCache.map(function (ele) {
                                if (ele.id === element.id) {
                                    ...
                                    try {
                                        for (var _iterator11 = ele.inflexionPoint[Symbol.iterator](), _step11; !(_iteratorNormalCompletion11 = (_step11 = _iterator11.next()).done); _iteratorNormalCompletion11 = true) {
    ...
    moveCanvasComplete: function moveCanvasComplete(event) {
        ...
        if (this.composeModel) {
            ...
            var nodeInArea = function nodeInArea(node) {
                return top <= node.shape.top && left <= node.shape.left && right >= node.shape.right && bottom >= node.shape.bottom;
            };
            var composeNodes = this.nodes.filter(nodeInArea);
            var lineInArea = function lineInArea(line) {
